...
class PersonalityPredictionModel(nn.Module):
    def __init__(self, au_dim, lstm_hidden_size, max_seq_length=700, latent_dim=512, max_nb_variables=19, max_timesteps=700, nb_classes=5):
    ...
    def __init__(self, au_file, max_length=700):
        ...
        for i in range(y, x+1):
            ...
            if (len(dataset) < self.max_length) and (len(dataset) > 40):
                ...
                dataset_au = dataset1.filter(regex='_r|_angle', axis=1)
...
def evaluate_regression(model, data_loader, device, is_train=True):
    total_mse1, total_mse2, total_mse3, total_mse4, total_mse5 = 0, 0, 0, 0, 0
    total = 0
    
    model.eval()
    with torch.no_grad():
        for action_units, lengths, personalities in data_loader:
            ...
            # Predictions for each personality trait (already continuous between 0 and 1)
            y_pred1 = output[:, 0]
            y_pred2 = output[:, 1]
            y_pred3 = output[:, 2]
            y_pred4 = output[:, 3]
            ...
            # True values
            true1 = personalities[:, 0]
            true2 = personalities[:, 1]
            true3 = personalities[:, 2]
            true4 = personalities[:, 3]
            true5 = personalities[:, 4]

            # Compute MSE for each personality trait
            total_mse1 += torch.sum((y_pred1 - true1) ** 2).item()
            total_mse2 += torch.sum((y_pred2 - true2) ** 2).item()
            total_mse3 += torch.sum((y_pred3 - true3) ** 2).item()
            total_mse4 += torch.sum((y_pred4 - true4) ** 2).item()
            total_mse5 += torch.sum((y_pred5 - true5) ** 2).item()
            total += personalities.size(0)

    # Calculate average MSE for each trait
    mse1 = total_mse1 / total
    mse2 = total_mse2 / total
    mse3 = total_mse3 / total
    mse4 = total_mse4 / total
    mse5 = total_mse5 / total

    if is_train:
        print(f"Train MSE: {mse1}, {mse2}, {mse3}, {mse4}, {mse5}")
    else:
        print(f"Test MSE: {mse1}, {mse2}, {mse3}, {mse4}, {mse5}")

    return mse1, mse2, mse3, mse4, mse5
# Set device
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Create dataset and data loader
train_dataset = FaceDataset('head_augmented.csv')
train_loader, test_loader = build_dataloader(train_dataset, batch_size=1, shuffle=True)

# Initialize the model, optimizer, and criterion
model = PersonalityPredictionModel(au_dim=19, lstm_hidden_size=128, max_seq_length=700).to(device)
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (action_units, lengths, personalities) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        outputs = model(action_units, lengths, timesteps)
        # print(output1,personalities)
        # print(torch.tensor([int(personalities[0][0]*4)]))
        
        loss = criterion(outputs, personalities)
    ...
    if epoch % save_interval == 0:
        ...
        evaluate_regression(model, train_loader, device, is_train=True)
        
        # Evaluate on the test set
        evaluate_regression(model, test_loader, device, is_train=False)
        # Save model
        torch.save(model.state_dict(), f"gaze_{epoch}.pt")
