...
class PersonalityPredictionModel(nn.Module):
    ...
    def forward(self, action_units, lengths, timesteps):
        ...
        x = torch.cat((x.squeeze(-1), x.squeeze(-1)), dim=1)
...
class FaceDataset(Dataset):
    def __init__(self, au_file, max_length=700):
        ...
        data_dict = {}
        new_name_list = []
        # dataset_complete=self.au_data
        au_columns = dataset_complete.filter(regex='_angle|_R').columns
        
        # Normalize the specified columns
        scaler = MinMaxScaler()
        dataset_complete[au_columns] = scaler.fit_transform(dataset_complete[au_columns])
...
model.load_state_dict(torch.load('user_sq_120.pt'))  # replace with your checkpoint file
model.eval()

# Create dataset and data loader
test_dataset = FaceDataset('head_augmented.csv')  # replace with your test dataset file
test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False)

actual_values = [[] for _ in range(5)]
...
with torch.no_grad():
    ...
    for action_units, lengths, personalities in test_loader:
        ...
        for i in range(5):
            predicted_values[i].extend(output[:, i].cpu().numpy())
            actual_values[i].extend(personalities[:, i].cpu().numpy())
        
        


for i in range(5):
    r2 = r2_score(actual_values[i], predicted_values[i])
