...
class PersonalityPredictionModel(nn.Module):
    def __init__(self, au_dim, lstm_hidden_size, max_seq_length=700, latent_dim=512, max_nb_variables=20, max_timesteps=700, nb_classes=5):
    ...
    def __init__(self, au_file, max_length=700):
        ...
        for i in range(y, x+1):
            ...
            if (len(dataset) < self.max_length) and (len(dataset) > 40):
                ...
                dataset_au = dataset1.filter(regex='_r|_R', axis=1)
...
model = PersonalityPredictionModel(au_dim=20, lstm_hidden_size=128, max_seq_length=700).to(device)
...
for epoch in range(num_epochs):
    ...
    if epoch % save_interval == 0:
        ...
        # Save model
        torch.save(model.state_dict(), f"rotation_{epoch}.pt")
