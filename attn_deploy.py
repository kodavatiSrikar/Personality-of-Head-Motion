...
model.load_state_dict(torch.load('3iternorm_180.pt'))  # replace with your checkpoint file
model.eval()

# Create dataset and data loader
test_dataset = FaceDataset('final_rotation.csv')  # replace with your test dataset file
...
with torch.no_grad():
    ...
    for action_units, lengths, personalities,ID in test_loader:
        ...
        y_pred4 = output[:, 3]
        y_pred5 = output[:, 4]
        pred_dict[int(ID[0])] = {'extroversion': y_pred1,
                            'neuroticism': y_pred2,
                            'agreeableness': y_pred3,
                            'conscientiousness': y_pred4,
                            'openness': y_pred5,
                                  }
    au_data = pd.read_csv('final_rotation.csv')
    # x = max(au_data['ID'])
    # y = min(au_data['ID'])
    # print(y,x)
